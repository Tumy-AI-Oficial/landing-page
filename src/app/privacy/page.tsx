...
export default function PrivacyPage() {
  ...
  const sections = t("privacyPage.sections") as { title: string; content: string; list: string[] }[];
        ...
        <BlurFade delay={0.1} inView>
          <div className="mb-12">
            <p className="text-xs font-medium tracking-[0.2em] uppercase text-neutral-400 mb-4 font-mono">
              {t("privacyPage.legalTag")}
            </p>
            <h1 className="text-3xl font-bold lg:text-5xl tracking-tight mb-6">
              {t("privacyPage.title")}
            </h1>
            <p className="text-sm text-neutral-500 dark:text-neutral-400 font-mono">
              {t("privacyPage.lastUpdated")}{new Date().toLocaleDateString(locale === 'es' ? 'es-ES' : 'en-US', { month: 'long', year: 'numeric' })}
            </p>
          </div>
        </BlurFade>
